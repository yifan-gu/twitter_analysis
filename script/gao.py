...
def load(tid, uid, ts, tx, rt):
    ...
    def barr2str(arr):
        return "".join("\\x{:02X}".format(ord(i)) for i in arr)
...
def cao(tweet_json):
    ...
    # text
    text = tweet['text']
    #print "".join("\\x{:02X}".format(ord(i)) for i in text.encode('utf8'))
    text = "".join("\\x{:02X}".format(ord(i)) for i in text.encode('utf8'))
    #sp = (tweet['text'].encode('utf8')
                      #.replace('\\','\\\\')
                      #.replace('"', '\\"')
                      #.split('\n') )
    #text = "\\n".join(sp)
